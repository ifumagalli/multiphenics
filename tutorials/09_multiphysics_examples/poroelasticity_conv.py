...
for ii in range(1,5):
    ...
    form_err_u_L2 = inner(uP_h - dP_ex, uP_h - dP_ex) * dx(poroel)
    form_err_u_H10 = inner(sym(grad(uP_h)) - symgrad_dP_ex, sym(grad(uP_h)) - symgrad_dP_ex) * dx(poroel)
    form_err_p_L2 = (pP_h - pP_ex)*(pP_h - pP_ex) * dx(poroel)
    form_err_p_H10 = inner(grad(pP_h) - grad_pP_ex, grad(pP_h) - grad_pP_ex) * dx(poroel)
    err_u_L2 = sqrt(assemble(form_err_u_L2))
    err_u_H10 = sqrt(assemble(form_err_u_H10))
    err_p_L2 = sqrt(assemble(form_err_p_L2))
    err_p_H10 = sqrt(assemble(form_err_p_H10))
